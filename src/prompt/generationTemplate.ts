...
export const buildVeoPrompt = (context: GenerationContext): string => {
  ...
  const tone =
    brand.toneDescription ??
    "Maintain confident, upbeat tone consistent with the brand.";
...
${
  ...
  const iterationLabel =
    iteration === 1 ? "Initial concept" : `Refinement pass #${iteration}`;

  return `You are Veo, generating a ${iterationLabel} for ${
    brand.name
  }.${userGuidance}
...
Requirements:
- CRITICAL #1: The PRODUCT (${
    campaign.productDescription
  }) MUST be physically visible in the video. If the product is not shown, this is an automatic FAIL.
- CRITICAL #2: Any text showing the brand name MUST spell "${
    brand.name
  }" EXACTLY. Check every letter.
...
- Ensure logo appears cleanly in final frame with correct spelling "${
    brand.name
  }".
